...
const NewsCard = ({ variant, image, title, author, date }: NewsCardProps) => {
  ...
  return (
    <div
      className={`${currentStyle.container} bg-[#020202] rounded-2xl overflow-hidden hover:scale-[1.02] xl:hover:scale-104 transition-transform duration-300 cursor-pointer shadow-lg flex flex-col`}
    >
      <div
      ...
      <div
        className={`${currentStyle.contentPadding} flex flex-col justify-between flex-1`}
      >
        <div
          className={`${currentStyle.titleSize} font-medium leading-tight text-white mb-2 xl:mb-1`}
        >
          {title}
        </div>

        <div
          className={`flex ${currentStyle.metaGap} ${currentStyle.metaSize} text-[#797979] font-light`}
        >
          <div>{author}</div>
          <div>{date}</div>
        </div>
