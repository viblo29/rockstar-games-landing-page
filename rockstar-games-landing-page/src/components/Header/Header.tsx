...
const HamburgerButton = ({ isOpen, onClick }: HamburgerButtonProps) => {
  return (
    ...
    >
      <span
        className={`block h-0.5 w-8 bg-white rounded-full transition-all duration-300 ease-in-out origin-center ${
          isOpen ? "rotate-45 translate-y-2" : ""
        }`}
      ></span>

      <span
        className={`block h-0.5 w-8 bg-white rounded-full transition-all duration-300 ease-in-out ${
      ...
      <div className="h-20 xl:h-28 w-full max-w-[1440px] px-6 xl:px-[140px] flex justify-between items-center relative">
        <div className="flex gap-0 xl:gap-[47px] items-center">
          ...
          <div className="hidden xl:flex items-center gap-6">
            ...
            <Link
              to="/loja"
              className="cursor-pointer hover:text-[#FCAF17] transition-colors duration-300"
            >
              Loja
            </Link>
            <Link
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-[#020202] z-60 flex flex-col p-6 animate-in slide-in-from-right duration-300 xl:hidden">
          ...
          <div className="flex flex-col gap-6 text-xl font-medium">
            ...
            <Link
              to="/loja"
              className="cursor-pointer hover:text-[#FCAF17]"
