...
function FeaturedGames() {
  ...
  const GameCard = ({
    ...
    <div
      onClick={onClick}
      className="w-[260px] h-[315px] cursor-pointer transition-transform duration-300 hover:scale-105 hover:shadow-lg"
    >
    ...
    <div className="flex w-full justify-center">
      <div className="flex w-full max-w-[1440px] flex-col items-center px-4">
        <div className="flex w-full flex-col xl:flex-row justify-between items-center mb-10 gap-4">
          <div className="font-extrabold text-[32px] xl:text-[48px]">
            Jogos em Destaque
          </div>

          <div className="flex flex-wrap justify-center gap-2 xl:gap-4">
            <div className="bg-[#606060] px-4 py-2 xl:px-6 xl:py-0 xl:h-8 flex items-center rounded-full cursor-pointer transition duration-200 hover:bg-[#464646] text-sm xl:text-base">
              PC
            </div>
            <div className="bg-[#606060] px-4 py-2 xl:px-6 xl:py-0 xl:h-8 flex items-center rounded-full cursor-pointer transition duration-200 hover:bg-[#464646] text-sm xl:text-base">
              Console
        ...
        <div className="hidden xl:flex flex-col items-center w-full">
          <div className="mb-16 flex gap-10 flex-wrap justify-center">
            ...
            {showMore &&
              gamesData.slice(4).map((game) => (
                <div
                  key={game.id}
                  className="animate-[fadeIn_0.5s_ease-in_forwards]"
                >
                  <GameCard
                    src={game.src}
                    alt={game.alt}
          ...
          <button onClick={() => setShowMore(!showMore)}>
            <Button
              ...
              w="260px"
              h="63px"
              text={showMore ? "Ver menos" : "Ver todos os jogos"}
            />
          </button>
        </div>
